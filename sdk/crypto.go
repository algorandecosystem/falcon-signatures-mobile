...
func SignTransaction(sk []byte, encodedTx []byte) (stxBytes []byte, err error) {
	...
	if encodedTx == nil || len(encodedTx) == 0 {
		return nil, fmt.Errorf("encoded transaction is nil or empty")
	}
	...
	// Make defensive copies to avoid memory issues
	skCopy := make([]byte, len(sk))
	copy(skCopy, sk)

	txCopy := make([]byte, len(encodedTx))
	copy(txCopy, encodedTx)

	var tx types.Transaction
	err = msgpack.Decode(txCopy, &tx)
	if err != nil {
		return nil, fmt.Errorf("failed to decode transaction: %v", err)
	}

	_, stxBytes, err = crypto.SignTransaction(skCopy, tx)
	if err != nil {
